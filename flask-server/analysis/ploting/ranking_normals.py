...
def accVsCon(list_number: int, json_path: str):
    ...
    for word in twoDim_con_data:
        confidance.append(sum(word) / len(word))

    accuracy = []
    for word in twoDim_acc_data:
        accuracy.append(sum(word) / len(word))
    ...
    plt.xlim(0,1)
    # plt.xticks(rotation=90)
    plt.title('Per Word Acc vs Con, (Averaged Over Trials): {}'.format(name), fontweight='bold')
    plt.ylabel('Accuracy',fontweight='bold')
    plt.xlabel('Confidence', fontweight='bold')
    plt.text(0.18, 0.1, f"R^2 = {r2:.2f}", fontsize=12, bbox=dict(facecolor='red', alpha=0.5))
    plt.text(0.05, 0.1, f"y={coefficients[0]:.2f}x+{coefficients[1]:.2f}", bbox=dict(facecolor='red', alpha=0.5), fontsize=12)
